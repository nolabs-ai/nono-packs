...
```bash
nono run --profile <active-profile> --read /path/to/needed -- omp    # read-only access
nono run --profile <active-profile> --write /path/to/needed -- omp   # write-only access
nono run --profile <active-profile> --allow /path/to/needed -- omp   # only when both are required
```
